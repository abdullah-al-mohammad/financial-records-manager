...
const formatOrderDate = formatDisplayDate;
...
const formatOrderDetails = record => {
  const parts = [];
  if (record.salesType) parts.push(record.salesType);
  if (record.digitalPaymentMethod) parts.push(record.digitalPaymentMethod);
  if (record.expenseDescription) parts.push(record.expenseDescription);
  if (record.riderName) parts.push(`Rider: ${record.riderName}`);
...
export default function BillingManager({
  ...
  // Merchant bill report (date range + print)
  const [billReportMerchant, setBillReportMerchant] = useState('');
  const [billReportStartDate, setBillReportStartDate] = useState('');
  const [billReportEndDate, setBillReportEndDate] = useState('');
  ...
  const activeReportMerchant = billReportMerchant || focusedMerchant;
  const dueBillReminders = useMemo(() => getDueBillReminders(billReminders), [billReminders]);

  useEffect(() => {
    saveBillReminders(billReminders);
  }, [billReminders]);
  ...
  useEffect(() => {
    ...
    const newDueReminders = dueBillReminders.filter(
      reminder => !notifiedReminderIds.includes(reminder.id)
    );
    if (newDueReminders.length === 0) {
      return;
    }

    setNotifiedReminderIds(prev => [...prev, ...newDueReminders.map(item => item.id)]);
    ...
    if (Notification.permission === 'granted') {
      newDueReminders.forEach(reminder => {
        new Notification(`Merchant bill due: ${reminder.merchantName}`, {
          body: `${reminder.merchantName} bill is due today. Last paid on ${reminder.lastPaidDate}.`,
        });
      });
    }
  }, [dueBillReminders, notifiedReminderIds]);

  const merchantBillReportData = useMemo(() => {
    if (!activeReportMerchant) {
      return {
        records: [],
        payments: [],
        totals: { sales: 0, billed: 0, paid: 0, ending: 0 },
        totalMerchantRecords: 0,
      };
    }

    const startDate = billReportStartDate;
    const endDate = billReportEndDate;

    const allRecords = records
      .filter(r => merchantsMatch(r.merchantName, activeReportMerchant))
      .sort((a, b) => compareDates(a.date, b.date));

    const allPayments = payments
      .filter(p => merchantsMatch(p.merchantName, activeReportMerchant))
      .sort((a, b) => compareDates(a.date, b.date));
    ...
    return {
      ...
      totals: { sales, billed, paid, ending: billed - paid },
      totalMerchantRecords: allRecords.length,
    };
  }, [activeReportMerchant, records, payments, billReportStartDate, billReportEndDate]);
  ...
  const openMerchantStatement = merchantName => {
    setBillReportMerchant(merchantName);
    setFocusedMerchant(merchantName);
  };

  // Parse month from any date format
  const getMonthName = getMonthFromDate;
  ...
  const aggregateTotals = useMemo(() => {
    ...
    // Direct sum of payments in selected month (fallback if ledger month bucketing misses any)
    const directMonthPaid = payments.reduce((sum, p) => {
      const paymentMonth = getMonthFromDate(p.date);
    ...
    return {
      starting,
      billed,
      paid: directMonthPaid,
      ending,
    };
  }, [filteredLedger, payments, selectedMonth]);
  ...
  const handlePaymentSubmit = async e => {
    ...
    setBillReminders(current => upsertBillReminder(current, selectedMerchant, payload.date));
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-sm font-semibold text-white">Merchant bills are due today.</p>
            </div>
            <span className="rounded-full bg-amber-500/20 px-2.5 py-1 text-[10px] font-semibold text-amber-200">
              {dueBillReminders.length} due
            </span>
          </div>
          <div className="space-y-2">
            {dueBillReminders.map(reminder => (
              <div
                key={reminder.id}
                className="rounded-xl border border-amber-400/20 bg-slate-950/70 p-3"
              >
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <span className="font-semibold text-white">{reminder.merchantName}</span>
                  <span className="text-[10px] font-medium text-amber-200">
                    Due on {reminder.nextDueDate}
                  </span>
                </div>
                <p className="mt-1 text-[11px] text-slate-300">
                  Last payout logged on {reminder.lastPaidDate}. A follow-up merchant bill reminder
                  is due now.
                </p>
              </div>
            ))}
      ...
      <div className="glass-panel border border-slate-900 rounded-2xl p-5 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">
              Choose a merchant and bill date range to preview or print all orders in that period
            </p>
          </div>
          <button
            type="button"
            onClick={handlePrintReport}
            disabled={!billReportMerchant || !billReportStartDate || !billReportEndDate}
            className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white rounded-xl text-xs font-semibold cursor-pointer flex items-center gap-1.5 transition-all shrink-0"
          >
        ...
        {(billReportStartDate || billReportEndDate) && (
          <button
            type="button"
            onClick={() => {
              setBillReportStartDate('');
        ...
        {activeReportMerchant ? (
          <div className="space-y-3 border-t border-slate-900 pt-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <p className="text-[10px] text-slate-400">
                Showing orders for{' '}
                <span className="font-bold text-white">{activeReportMerchant}</span>
                {billReportStartDate && billReportEndDate && (
                  <span>
                    {' '}
                    from{' '}
                    <span className="text-indigo-400">
                      {formatOrderDate(billReportStartDate)}
                    </span>{' '}
                    to <span className="text-indigo-400">{formatOrderDate(billReportEndDate)}</span>
                  </span>
                )}
                {!billReportStartDate && !billReportEndDate && (
                  <span className="text-slate-500"> (all dates — set a range to filter)</span>
                )}
              </p>
              <span className="text-xs font-bold text-indigo-400">
                Total Bill: ৳{merchantBillReportData.totals.billed.toLocaleString()}
              </span>
            </div>
            <div className="overflow-x-auto border border-slate-900 rounded-xl max-h-72 overflow-y-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-950 text-slate-400 sticky top-0">
                  <tr>
                    <th className="p-2.5 whitespace-nowrap">Order Date</th>
                    <th className="p-2.5">Order Details</th>
                    <th className="p-2.5 text-right whitespace-nowrap">Amount</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-900/60 text-slate-300">
                  {merchantBillReportData.records.length === 0 ? (
                    <tr>
                      <td colSpan="3" className="p-4 text-center text-slate-500">
                        {merchantBillReportData.totalMerchantRecords === 0
                          ? `No sales records found for ${activeReportMerchant}.`
                          : billReportStartDate || billReportEndDate
                            ? `No orders in this date range. This merchant has ${merchantBillReportData.totalMerchantRecords} order(s) outside the selected dates.`
                            : 'No orders found for this merchant.'}
                      </td>
                    </tr>
                  ) : (
                    merchantBillReportData.records.map(r => (
                      <tr key={r.id}>
                        <td className="p-2.5 whitespace-nowrap">{formatOrderDate(r.date)}</td>
                        <td className="p-2.5">{formatOrderDetails(r)}</td>
                        <td className="p-2.5 text-right font-bold text-indigo-400 whitespace-nowrap">
          ...
          <p className="text-[10px] text-slate-500 italic border-t border-slate-900 pt-4">
            Select a merchant above to preview their bill report.
          </p>
        )}
      </div>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="flex-1 flex flex-col justify-between space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                    Cycle Overview Balances
                  </h3>
                  <p className="text-[10px] text-slate-500 mt-0.5">
                    Summary of dues and transfers for {selectedMonth}
                  </p>
                </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="p-4 bg-slate-950/60 border border-slate-900 rounded-xl">
                  ...
                  <span className="text-lg font-extrabold text-slate-300 block mt-1">
                    ৳{aggregateTotals.starting.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="p-4 bg-slate-950/60 border border-slate-900 rounded-xl">
                  ...
                  <span className="text-lg font-extrabold text-indigo-400 block mt-1">
                    ৳{aggregateTotals.billed.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="p-4 bg-slate-950/60 border border-slate-900 rounded-xl">
                  ...
                  <span className="text-lg font-extrabold text-emerald-400 block mt-1">
                    ৳{aggregateTotals.paid.toLocaleString()}
                  </span>
                </div>
                ...
                <div className="p-4 bg-slate-950/60 border border-slate-900 rounded-xl shadow-inner shadow-rose-500/5">
                  ...
                  <span className="text-lg font-extrabold text-rose-400 block mt-1">
                    ৳{aggregateTotals.ending.toLocaleString()}
                  </span>
                </div>
              </div>
              ...
              <div className="text-[11px] text-slate-500 bg-slate-950/40 p-3 rounded-xl border border-slate-900">
                {carryForward ? (
                  <span className="flex items-start gap-1.5">
                    ...
                    Dues carry-forward is active. The Ending Balance includes unpaid bills
                    accumulated from all preceding months.
                  </span>
                ) : (
                  <span className="flex items-start gap-1.5">
                    <CornerDownRight className="w-4 h-4 text-slate-600 flex-shrink-0" />
                    Isolated month calculations. Showing outstanding bills and payments generated
                    strictly during {selectedMonth}.
                  </span>
                )}
              </div>
            </div>
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-32 overflow-y-auto pr-1">
                  {billingLedger.filter(m => m.startingBalance > 0).length === 0 ? (
                    ...
                    billingLedger
                      .filter(m => m.startingBalance > 0)
                      .map(m => (
                        <div
                          key={m.name}
                          className="flex justify-between items-center text-[10px] py-1 border-b border-slate-900/40 last:border-b-0"
                        >
                          <span className="font-medium text-slate-300 truncate max-w-[120px]">
                            {m.name}
                          </span>
                          <span className="font-bold text-rose-400">
                            ৳{m.startingBalance.toLocaleString()}
                          </span>
                        </div>
                      ))
                  )}
            ...
            >
              ...
              <div className="pt-3 border-t border-slate-900 flex items-center justify-between">
                <div>
                  ...
                  <span
                    className={`text-base font-extrabold block mt-0.5 ${m.endingBalance > 0 ? 'text-rose-400' : 'text-emerald-400'}`}
                  >
      ...
      {focusedMerchant && (
        <div className="fixed inset-0 z-50 overflow-hidden flex justify-end">
          ...
          <div className="relative w-full max-w-2xl bg-[var(--drawer-bg)] border-l border-slate-900 h-full shadow-2xl flex flex-col justify-between z-10 animate-slide-in">
            {/* Header */}
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="p-3 bg-slate-950/40 border border-slate-900 rounded-xl text-center">
                  ...
                  <span className="text-sm font-extrabold text-indigo-400 block mt-1">
                    ৳{merchantBillReportData.totals.billed.toLocaleString()}
                  </span>
                </div>
                <div className="p-3 bg-slate-950/40 border border-slate-900 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block">
                  ...
                  <span className="text-sm font-extrabold text-emerald-400 block mt-1">
                    ৳{merchantBillReportData.totals.paid.toLocaleString()}
                  </span>
                </div>
                <div className="p-3 bg-slate-950/40 border border-slate-900 rounded-xl text-center">
                  <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wider block">
                  ...
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-slate-900/60 text-slate-300">
                      {merchantBillReportData?.payments.length === 0 ? (
                        ...
                        merchantBillReportData?.payments.map(p => (
                          <tr key={p.id}>
                            <td className="p-2.5">{p.date ? p.date.slice(0, 10) : '—'}</td>
                            <td className="p-2.5 font-bold text-emerald-400">
                              ৳{Number(p.paidAmount).toLocaleString()}
                            </td>
                            <td className="p-2.5 text-slate-500 italic truncate max-w-xs">
                              {p.notes || '—'}
                            </td>
                          </tr>
                        ))
      ...
      <div className="printable-report-area hidden bg-white text-black p-8 font-sans w-full max-w-4xl mx-auto">
        <div className="flex justify-between items-start border-b-2 border-slate-300 pb-4 mb-6">
          <div className="flex flex-col gap-4">
            <div 
              className="bg-black inline-flex p-2.5 rounded-xl shadow-sm self-start"
          ...
          <div className="text-right">
            <h2 className="text-base font-bold text-slate-900">{activeReportMerchant}</h2>
            <p className="text-[10px] text-slate-500 mt-1 font-medium">
              Period: {billReportStartDate ? formatOrderDate(billReportStartDate) : 'All History'}{' '}
              to {billReportEndDate ? formatOrderDate(billReportEndDate) : 'Present'}
            </p>
          </div>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-200">
              {merchantBillReportData?.records.length === 0 ? (
                ...
                merchantBillReportData?.records.map(r => (
                  <tr key={r.id}>
                    <td className="py-2 px-3">{formatOrderDate(r.date)}</td>
                    <td className="py-2 px-3 text-slate-700">{formatOrderDetails(r)}</td>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-slate-200">
              {merchantBillReportData?.payments.length === 0 ? (
                <tr>
                  <td colSpan="3" className="py-4 px-3 text-center text-slate-500 italic">
                    No payouts recorded in this period.
                  </td>
                </tr>
              ) : (
                merchantBillReportData?.payments.map(p => (
        ...
        <div className="grid grid-cols-2 gap-8 border-t border-slate-300 pt-6 mt-8">
          ...
          <div className="bg-slate-50 rounded-xl p-4 space-y-2 border border-slate-200">
            ...
            <div className="flex justify-between text-sm border-t border-slate-300 pt-2 font-bold text-slate-900">
              ...
              <span
                className={
                  merchantBillReportData?.totals.ending > 0 ? 'text-rose-700' : 'text-emerald-700'
                }
              >
                ৳{merchantBillReportData?.totals.ending.toLocaleString()}
              </span>
